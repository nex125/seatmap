...
var createSeatmapStore = () => zustand.create((set) => ({
  venue: null,
...
var SeatmapContext = react.createContext(null);
function useSeatmapContext() {
  const ctx = react.useContext(SeatmapContext);
  if (!ctx) {
...
function SeatmapProvider({ venue, children }) {
  const storeRef = react.useRef(createSeatmapStore());
  const viewportRef = react.useRef(new seatmapCore.Viewport());
  const spatialIndexRef = react.useRef(new seatmapCore.SpatialIndex());
  react.useEffect(() => {
    if (venue) {
  ...
  return /* @__PURE__ */ jsxRuntime.jsx(SeatmapContext.Provider, { value: contextValue, children });
}
var SEAT_RADIUS = 7;
function SeatmapCanvas({
  ...
  const textureCacheRef = react.useRef(new seatmapCore.CategoryTextureCache());
  ...
  const bgTextureRef = react.useRef(null);
  const bgUrlRef = react.useRef("");
  const touchesRef = react.useRef(/* @__PURE__ */ new Map());
  const lastPinchDistRef = react.useRef(null);
  const lastPinchCenterRef = react.useRef(null);
  const touchTapRef = react.useRef(null);
  ...
  const hoveredSeatId = zustand.useStore(store, (s) => s.hoveredSeatId);
  ...
  const getSeatTexture = react.useCallback(
  ...
  const zoomToSection = react.useCallback(
    (section) => {
  ...
  const renderGAArea = react.useCallback(
    (parent, ga) => {
  ...
  const renderSection = react.useCallback(
    (parent, section, lod, _visibleAABB) => {
      ...
      } else if (lod === seatmapCore.LODLevel.Section) {
        if (section.outline.length > 2) {
  ...
  const renderScene = react.useCallback(() => {
    ...
    const lod = seatmapCore.getLODLevel(zoom);
    world.position.set(viewport.x * zoom, viewport.y * zoom);
    world.scale.set(zoom);
    const boundsG = new pixi_js.Graphics();
    ...
    if (bgTextureRef.current && bgTextureRef.current !== pixi_js.Texture.EMPTY) {
      const bgSprite = new pixi_js.Sprite(bgTextureRef.current);
  ...
  react.useEffect(() => {
    ...
    app.init({
      ...
      appRef.current = app;
      const worldContainer = new pixi_js.Container();
      ...
      if (currentVenue) {
        ...
        viewport.fitBounds(seatmapCore.venueAABB(currentVenue));
      }
  ...
  const prevVenueIdRef = react.useRef(null);
  const prevCatJsonRef = react.useRef("");
  react.useEffect(() => {
  ...
  react.useEffect(() => {
    ...
    pixi_js.Assets.load({ src: url, alias: cacheKey }).then((tex) => {
      if (cancelled) return;
...
function DefaultTooltip({ data }) {
  return /* @__PURE__ */ jsxRuntime.jsxs(
    "div",
    {
      ...
      children: [
        /* @__PURE__ */ jsxRuntime.jsx("div", { style: { fontWeight: 600, marginBottom: 2 }, children: data.section.label }),
        ...
        /* @__PURE__ */ jsxRuntime.jsx("div", { style: { color: "#9e9e9e", fontSize: 12, marginTop: 2 }, children: data.seat.status === "available" ? "Available" : data.seat.status })
      ]
    }
  );
}
function TooltipOverlay({ renderTooltip, style }) {
  ...
  react.useEffect(() => {
    ...
    for (const section of venue.sections) {
      for (const row of section.rows) {
        ...
        if (seat) {
          const worldPos = seatmapCore.seatWorldPosition(section, seat);
  ...
  react.useEffect(() => {
    ...
    const unsub = viewport.subscribe(() => {
      ...
      if (!section) return;
      const worldPos = seatmapCore.seatWorldPosition(section, tooltipData.seat);
      const screenPos = viewport.worldToScreen(worldPos.x, worldPos.y);
      setTooltipData(
  ...
  return /* @__PURE__ */ jsxRuntime.jsx(
    "div",
    {
      ...
      children: renderTooltip ? renderTooltip(tooltipData) : /* @__PURE__ */ jsxRuntime.jsx(DefaultTooltip, { data: tooltipData })
    }
  );
}
function Minimap({ width = 180, height = 120, style }) {
  ...
  react.useEffect(() => {
    ...
    const draw = () => {
      ...
      const bounds = seatmapCore.venueAABB(venue);
      ...
      for (const section of venue.sections) {
        const box = seatmapCore.sectionAABB(section);
...
function useViewport() {
  ...
  const fitToVenue = react.useCallback(() => {
...
function useSelection() {
  ...
  const selectedSeatIds = zustand.useStore(store, (s) => s.selectedSeatIds);
  const toggleSeat = zustand.useStore(store, (s) => s.toggleSeat);
  const clearSelection = zustand.useStore(store, (s) => s.clearSelection);
  const setSelection = zustand.useStore(store, (s) => s.setSelection);
  return { selectedSeatIds, toggleSeat, clearSelection, setSelection };
}
function useSeatmap() {
  const { store, viewport, spatialIndex } = useSeatmapContext();
  const venue = zustand.useStore(store, (s) => s.venue);
  const setVenue = zustand.useStore(store, (s) => s.setVenue);
  const hoveredSeatId = zustand.useStore(store, (s) => s.hoveredSeatId);
  const setHoveredSeat = zustand.useStore(store, (s) => s.setHoveredSeat);
  return { venue, setVenue, hoveredSeatId, setHoveredSeat, viewport, spatialIndex };
}
function useSeatStatus() {
  const { store } = useSeatmapContext();
  const updateSeatStatus = react.useCallback(
    (seatId, status) => {
      const venue = store.getState().venue;
      if (!venue) return;
  ...
  const updateBulkStatus = react.useCallback(
    (updates) => {
...
exports.SeatmapCanvas = SeatmapCanvas;
exports.SeatmapContext = SeatmapContext;
exports.SeatmapProvider = SeatmapProvider;
exports.TooltipOverlay = TooltipOverlay;
exports.createSeatmapStore = createSeatmapStore;
exports.useSeatStatus = useSeatStatus;
exports.useSeatmap = useSeatmap;
exports.useSeatmapContext = useSeatmapContext;
